...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(args[0]) {
            ...
            .addField("Fun:", "``8ball`` ``advice`` ``anime`` ``asciify`` ``clyde`` ``decrypt`` ``didumean`` ``emoji`` ``encrypt`` ``facts`` ``flip`` ``fml`` ``gayrate`` ``hack`` ``howdumb`` ``hug`` ``kill`` ``kiss`` ``meme`` ``morse`` ``oof`` ``pickup`` ``rate`` ``roll`` ``slap`` ``trump`` ``yomama``")
            ...
            .addField("Utilities:", "``bug`` ``calculate`` ``color`` ``dm`` ``giphy`` ``hastebin`` ``report`` ``say`` ``shorten`` ``suggest`` ``translate``")
